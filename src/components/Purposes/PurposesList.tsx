...
import { checkedListAtom, purposesListAtom, totalPageAtom } from "@/atom";
...
import { deletePurposes, getPurposesList } from "@/hooks/useData";

interface Props {
  url?: string;
}

const PurposesList = ({ url }: Props) => {
  ...
  const [itemsList, setItemsList] = useRecoilState(purposesListAtom);
  ...
  const itemDelete = async () => {
    ...
    checkedElements.forEach(async (element) => {
      await deletePurposes(String(userToken), Number(element));
    });
    getData();
    setChechedElements([]);
    setIsOpen(false);
  ...
  const getData = async () => {
    const userToken = getToken();
    const response = await getPurposesList(String(userToken));
            ...
            {itemsList?.map((item, index) => (
              <tr key={index}>
                <td className="border-b  border-[#eee] px-3 py-4  dark:border-strokedark ">
                  <label
                    htmlFor={String(item?.purposeId)}
                    className="flex cursor-pointer select-none items-center"
                  >
                    <div className="relative">
                      <input
                        type="checkbox"
                        id={String(item?.purposeId)}
                        className="sr-only"
                        onChange={(e) =>
                          handleCheck(e, (item?.purposeId as unknown) as string)
                        }
