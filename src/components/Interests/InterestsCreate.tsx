...
import { createInterests } from "@/hooks/useData";
...
const InterestsCreate = ({ url }: Props) => {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    const res = await createInterests(String(token), title);
    if (res?.status) {
      setIsOpen(true);
      setLoading(false);
    } else {
...
export default InterestsCreate;
