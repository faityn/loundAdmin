...
import { getExhibitionList } from "@/hooks/useEvents";
...
const ExhibitionList = () => {
  ...
  const [exhibitionList, setExhibitionList] = useRecoilState(exhibitionListAtom);
  ...
  const getData = async () => {
    ...
    setExhibitionList(response?.products
    );
  };
  useEffect(() => {
          ...
          {isOpen ? (
            <CustomModal>
              <h2 className="text-xl text-black">
                ({checkedElements?.length}) {checkedElements?.length > 1 ? 'Users' : 'User' } will <br /> be
                deleted
              </h2>
            ...
            >
              <div className="relative">
                <input
                  ...
                  checked={
                    checkedElements.length === exhibitionList?.length ? true : false
                  }
                />
                <div
            ...
            <tr key={index}>
              ...
              >
                <div className="relative">
                  ...
                  <div
                    className={`mr-4 flex h-4 w-4 items-center justify-center rounded border ${
                        checkedElements.includes((item?.id as unknown) as string) &&
                      "border-primary bg-gray dark:bg-transparent"
                    }`}
                  >
                    <span
              ...
              <td className="border-b border-[#eee] px-4 py-4 dark:border-strokedark">
                ...
                {item?.meta?.createdAt
                  ? format(item?.meta?.createdAt as string, "yyyy-MM-dd HH:mm:ss")
                  : ""}
                </p>
              </td>
              <td className="border-b border-[#eee] px-4 py-4 dark:border-strokedark">
