"use client";
import { menuPermissionAtom, noticeDetailAtom } from "@/atom";
...
import { getNoticeDetail, updateNotice } from "@/hooks/useData";
...
const Update = ({ id, url }: Props) => {
  ...
  const [itemsDetail, setItemsDetail] = useRecoilState(noticeDetailAtom);
  ...
  const getData = async () => {
    ...
    const response = await getNoticeDetail(String(userToken), id);
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    if (contentValue === "") {
      ...
      const res = await updateNotice(String(token), id, title, content);
