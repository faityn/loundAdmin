...
const AdminUpdate = ({ id, url }: Props) => {
  ...
  const [adminDetail, setAdminDetail] = useRecoilState(adminDetailAtom);
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    const password = data.pass !== "" ? String(encrypt(data.pass)) : "";
  ...
  const getData = async () => {
    ...
    const res = await getAdminDetail(String(userToken), id);

    setAdminDetail(res);
    setValue("username", res?.username);
    setValue("status", res?.status);
    setUseStatus(res?.status);

    const allIds = res?.menues?.map((data: { menuId: number }) => {
      return data?.menuId;
    });
    setChechedElements(() => ([...allIds] as unknown) as string[]);

    const permittionArray = res?.menues?.map(
      (data: { menuId: number; status: string }) => {
        return { menuId: data?.menuId, status: data?.status };
      }
    );
    setAdminPermittionArray(() => [...permittionArray]);
                  ...
                  <div className="mb-4">
                    ...
                    <div>
                      <input
                        ...
                        defaultValue={adminDetail?.username}
                        className="w-full rounded border-[1.5px] border-slate-300 bg-transparent px-4 py-1.5 text-black dark:text-white outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter "
                      />
                      {errors.username && (
                              ...
                              <td className="w-30 pl-4 pt-5 border-b  border-[#eee]">
                                {menu?.children?.map((sub, s) => (
                                  <select
                                    ...
                                    value={`${
                                      adminPermittionArray.find(
                                        (item) => item.menuId === sub?.menuId
                                      )?.status
                                    }`}
                                    onChange={(e) =>
                                      handleSubPermittion(
